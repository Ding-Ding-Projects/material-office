...
export function renderComponents(ctx) {
  ...
  return `<section class="surface" data-surface="components" data-appearance-id="surface:components">
    ...
    <div class="surface-content component-grid">
      ...
      <article class="component-demo" data-appearance-id="component:fields"><h2>${escapeHtml(t('components.fields'))}</h2><label class="field"><span>${escapeHtml(localized(ctx, 'Project name', '專案名稱'))}</span><input value="${escapeAttribute(demo.field)}" data-action="demo-field" placeholder="Material Office"></label><label class="field"><span>${escapeHtml(localized(ctx, 'Density', '密度'))}</span><select data-action="set-density"><option value="compact" ${state.preferences?.density === 'compact' ? 'selected' : ''}>${escapeHtml(localized(ctx, 'Compact', '精簡'))}</option><option value="comfortable" ${state.preferences?.density === 'comfortable' ? 'selected' : ''}>${escapeHtml(localized(ctx, 'Comfortable', '舒適'))}</option></select></label></article>
      <article class="component-demo" data-appearance-id="component:progress"><h2>${escapeHtml(t('components.progress'))}</h2><label class="field"><span>${escapeHtml(localized(ctx, 'Progress', '進度'))} · <output data-demo-slider-value>${Number(demo.slider)}</output>%</span><input type="range" min="0" max="100" value="${Number(demo.slider)}" data-action="demo-slider"></label><progress max="100" value="${Number(demo.slider)}" style="width:100%;accent-color:var(--primary)"></progress></article>
      <article class="component-demo" data-appearance-id="component:tokens" style="grid-column:1/-1"><h2>${escapeHtml(t('components.tokens'))}</h2><div class="card-grid">${swatches.map(([name, background, color]) => `<button class="card" data-action="copy-token" data-token="${escapeAttribute(name)}" data-token-value="${escapeAttribute(background)}" aria-label="${escapeAttribute(localized(ctx, 'Copy {name} token {value}', '複製 {name} token {value}', { name, value: background }))}" style="min-height:92px;padding:14px;background:${background};color:${color};text-align:left"><strong>${escapeHtml(name)}</strong><small style="display:block;margin-top:6px">${escapeHtml(background)}</small></button>`).join('')}</div></article>
    </div>
  </section>`;
}
